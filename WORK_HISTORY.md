...
- Updated the board setup button text and safety-limit status message to say `Auto Safe 38%`.
- Commit status: committed locally; final console response reports the resulting hash.

What to test:
- Start a fresh Dots and Boxes match and click `Auto Safe 38%`.
- Confirm random blue auto lines appear, the button disables, and no box is left ready to be claimed in one move.

Verification:
- `node --check games/dots-and-boxes/logic.js`
- `node --check games/dots-and-boxes/ui.js`
- Node auto-line smoke test confirming a 38% target and no three-sided boxes.
- `git diff --check`
- `git diff --cached --check`

Manual browser verification was not run; this was a scoped target-value pass.

## 2026-05-13 - Dots and Boxes Auto Line Target 30

- Changed Dots and Boxes automatic safe-line seeding from a 25% target to a 30% target.
- Updated the board setup button text and safety-limit status message to say `Auto Safe 30%`.
- Commit status: committed locally; final console response reports the resulting hash.

What to test:
- Start a fresh Dots and Boxes match and click `Auto Safe 30%`.
- Confirm random blue auto lines appear, the button disables, and no box is left ready to be claimed in one move.

Verification:
- `node --check games/dots-and-boxes/logic.js`
- `node --check games/dots-and-boxes/ui.js`
- Node auto-line smoke test confirming a 30% target and no three-sided boxes.
- `git diff --check`
- `git diff --cached --check`

Manual browser verification was not run; this was a scoped target-value pass.

## 2026-05-13 - Dots and Boxes Auto Line Target 25

- Changed Dots and Boxes automatic safe-line seeding from a 45% target to a 25% target.
- Updated the board setup button text and safety-limit status message to say `Auto Safe 25%`.
- Commit status: committed locally; final console response reports the resulting hash.

What to test:
- Start a fresh Dots and Boxes match and click `Auto Safe 25%`.
- Confirm random blue auto lines appear, the button disables, and no box is left ready to be claimed in one move.

Verification:
- `node --check games/dots-and-boxes/logic.js`
- `node --check games/dots-and-boxes/ui.js`
- Node auto-line smoke test confirming a 25% target and no three-sided boxes.
- `git diff --check`
- `git diff --cached --check`

Manual browser verification was not run; this was a scoped target-value pass.

## 2026-05-13 - Dots and Boxes Auto Line Target 45

- Changed Dots and Boxes automatic safe-line seeding from a 50% target to a 45% target.
- Updated the board setup button text and safety-limit status message to say `Auto Safe 45%`.
- Commit status: committed locally; final console response reports the resulting hash.

What to test:
- Start a fresh Dots and Boxes match and click `Auto Safe 45%`.
- Confirm random blue auto lines appear, the button disables, and no box is left ready to be claimed in one move.

Verification:
- `node --check games/dots-and-boxes/logic.js`
- `node --check games/dots-and-boxes/ui.js`
- Node auto-line smoke test confirming a 45% target and no three-sided boxes.
- `git diff --check`
- `git diff --cached --check`

Manual browser verification was not run; this was a scoped target-value pass.

## 2026-05-13 - Dots and Boxes Auto Line Target 50

- Changed Dots and Boxes automatic safe-line seeding from a 60% target to a 50% target.
- Updated the board setup button text and safety-limit status message to say `Auto Safe 50%`.
- Commit status: committed locally; final console response reports the resulting hash.

What to test:
- Start a fresh Dots and Boxes match and click `Auto Safe 50%`.
- Confirm random blue auto lines appear, the button disables, and no box is left ready to be claimed in one move.

Verification:
- `node --check games/dots-and-boxes/logic.js`
- `node --check games/dots-and-boxes/ui.js`
- Node auto-line smoke test confirming a 50% target and no three-sided boxes.
- `git diff --check`
- `git diff --cached --check`
...
- Made `2x Lock` lock one non-full column for both players immediately, lasting four player turns total so each player is denied the lane twice.
- Reweighted the random power deck so Bomb Piece and 2x Lock are slightly less common than the standard powers.
- Updated Connect Four Plus rules/help text and lock readouts for the new both-player lock state.

What to test:
- In Power Duel or Token Hunt, get a `2x Lock`, select a column, and confirm neither player can play that column until both players have taken two turns.
- Confirm the selected column shows as actively locked, not prelocked, for both players.
- Confirm normal `Lock` still prelocks for the opponent and still lets the locking player use that lane on the same turn.

Verification:
- `node --check games/connect-four-plus/logic.js`
- `node --check games/connect-four-plus/ui.js`
- Node targeted Connect Four Plus smoke test covering power deck weights, 2x Lock duration/enforcement/clearing, and unchanged normal Lock behavior.
- `git diff --check`
